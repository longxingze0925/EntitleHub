...
### 24.3 ai_wallets 与 ai_wallet_ledger_entries
...
后台手动充值/扣减使用 `credit` / `debit`。网关转发时使用 `hold`、`capture`、`release`、`refund` 完成“发起请求先预扣，失败退款”的闭环。`daily_spend_limit_minor` 为空表示不限制；设置后控制该客户钱包当日新请求预扣金额上限。
...
### 24.4.1 server_api_keys

Web 后端、业务服务端调用 EntitleHub 使用的应用级 Server Key。Server Key 绑定 `app_id`，不绑定单个客户；调用时必须通过请求头传入本次消费的 `customer_id`，服务端再校验该客户在该应用下有有效订阅。
...
### 24.5 ai_usage_records 与 ai_assets
...
幂等请求通过 `(tenant_id, customer_id, api_key_id, endpoint, idempotency_key)` 唯一索引约束，避免客户端重试重复扣费。`ai_assets.status` 支持 `caching`、`ready`、`failed`、`deleted`；后台删除素材是软删除，客户端素材访问只返回 `ready` 状态。
